...
final public class AutoRunFromConsole {
     ...
     *                         Windows), the console will not close once the executable has terminated. This is usefule
    ...
    private static void startExecutableInConsole(final String executableName, final boolean stayOpenAfterEnd) {
        ...
        switch (getOsType()) {
            ...
            case WINDOWS:
                if (stayOpenAfterEnd) {
                    launchString = "cmd /c start cmd /k java -jar " + executableName;
                } else {
                    launchString = "cmd /c start java -jar " + executableName; // No, using /k directly here DOES NOT work.
                }
                break;
            case LINUX:
                break;
     ...
     * @return if name is not null and points to a FILE existing in the CURRENT directory.
     */
